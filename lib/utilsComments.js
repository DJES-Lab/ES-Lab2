                    ...
                    return [
                        ...
                        execQuery('sort', [
                            ...
                            .then(function(messages) {
                                var len = messages.length / 4;
                                var comments = new Array(len);
                                for (var i = 0; i < len; i++) {
                                    comments[i] = {
                                        name: messages[4 * i],
                                        input: messages[4 * i + 1],
                                        sticker: messages[4 * i + 2],
                                        time: messages[4 * i + 3]
                                    };
                                }
                                return comments;
            ...
            .then(function(commentNum) {
                execQuery('lpush', ['all:comments', commentNum + 1])
                    .then(function(newCommentId) {
                        ...
                        var hashPair = [commentName, 'name', req.body.name, 'input', req.body.input, 'sticker', req.body.sticker, 'time', new Date().toString()];
